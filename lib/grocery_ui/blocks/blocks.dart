import 'package:app/grocery_ui/blocks/product_grid.dart';
import 'package:app/grocery_ui/blocks/product_scroll.dart';
...
import 'package:app/src/ui/blocks/stores/store_card_list.dart';
import 'package:app/src/ui/blocks/stores/store_card_scroll.dart';
import 'package:app/src/ui/blocks/stores/store_list.dart';
...
class SliverBlock extends StatelessWidget {
  final Block block;
  final bool? vendorHome;
  final List<Category>? vendorCategories;
  SliverBlock({Key? key, required this.block, this.vendorHome, this.vendorCategories}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    switch (block.blockType) {
      ...
      case BlockType.categoryGrid:
        if(vendorHome == true) {
          List<Category> categories = vendorCategories != null ? vendorCategories!.where((cat) => cat.parent == block.linkId).toList() : [];
      ...
      case BlockType.productScroll:
        if(block.products.length > 0) {
          return ProductScroll(products: block.products, block: block);
        } else {
          return SliverToBoxAdapter();
        }
