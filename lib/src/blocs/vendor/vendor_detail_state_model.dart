...
class VendorDetailStateModel extends Model {
  ...
  void getDetails() async {
    final response = await apiProvider.post(
        '/wp-admin/admin-ajax.php?action=build-app-online-vendor_details',
        filter);
  ...
  void getReviews() async {
    ...
    final response = await apiProvider.post(
        '/wp-admin/admin-ajax.php?action=build-app-online-vendor_reviews',
        reviewsFilter);
  ...
  submitContactForm(data) async {
    final response = await apiProvider.post(
        '/wp-admin/admin-ajax.php?action=build-app-online-contact_vendor', data);
    
  }

  Future<void> submitReview(Map reviewData) async {
    reviewData['vendor'] = filter['vendor'];
    reviewData['wcfm_store_review_form'] = 'wcfm_store_review_category%5B0%5D=' + reviewData['rating'] + '&wcfm_review_store_id=' + filter['vendor']! + '&wcfm_review_author_id=' + AppStateModel().blocks.user.id.toString() + '&wcfmmp_store_review_comment=' + reviewData["comment"];
    final response = await apiProvider.post(
        '/wp-admin/admin-ajax.php?action=build-app-online-add_vendor_review',
        reviewData);
  }
