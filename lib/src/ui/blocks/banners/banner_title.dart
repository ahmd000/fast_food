...
class BannerTitle extends StatelessWidget {
      ...
      return Padding(
        ...
        child: block.headerAlign != 'top_left' || filter == null ? Align(
          ...
          child: ListTile(
            ...
            subtitle: block.description != null ? Text(block.description!, style: Theme.of(context).textTheme.bodyMedium!.copyWith(color:  Theme.of(context).brightness == Brightness.light ? block.titleColor : null)) : null,
          ),
        ) : ListTile(
          contentPadding: EdgeInsets.all(0),
          title: Row(
            ...
            children: [
              ...
              (block.blockType == BlockType.storeList || block.blockType == BlockType.storeSlider || block.blockType == BlockType.storeListTile || block.blockType == BlockType.storeScroll || block.blockType == BlockType.productGrid || block.blockType == BlockType.productScroll || block.blockType == BlockType.productSlider || block.blockType == BlockType.productList ||
                  block.blockType == BlockType.brandGrid || block.blockType == BlockType.brandList || block.blockType == BlockType.brandListTile || block.blockType == BlockType.brandPresets || block.blockType == BlockType.brandScroll || block.blockType == BlockType.brandSlider) ?
              TextButton(
  ...
  _onClickViewAll(BuildContext context, filter) {
    ...
    }  else if(block.blockType == BlockType.categoryGrid || block.blockType == BlockType.categoryList || block.blockType == BlockType.categoryScroll || block.blockType == BlockType.categorySlider || block.blockType == BlockType.categoryListTile || block.blockType == BlockType.categoryPresets) {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => AllBrands(title: block.title)));
