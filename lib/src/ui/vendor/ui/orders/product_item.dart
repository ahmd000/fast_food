...
class ProductItem extends StatefulWidget {
  ...
  ProductItem({Key? key, required this.vendorBloc, required this.product, required this.order, required this.variation})
      : super(key: key);
  @override
...
class _ProductItemState extends State<ProductItem> {
  ...
  void _addProduct() {
    if (widget.product.type == 'variable') {
      ...
      final lineItem = LineItem.fromJson({'productId': widget.product.id, 'variationId': widget.variation.id, 'quantity': 1, 'name': widget.product.name, 'price': double.parse(widget.variation.price), 'total': (1 * double.parse(widget.variation.price)).toString()});
      setState(() {
